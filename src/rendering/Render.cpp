...
void Render::render(sf::RenderWindow& window, Light light) {
    std::vector<Triangle> projectedTriangles, renderedTriangles;  // Треугольники после проекции и отсечения
    std::vector<sf::VertexArray> texturedTriangles;  // Текстурированные треугольники
    ...
    sf::Color edgeColor(255, 128, 0);  // Цвет рёбер
    ...
    for (auto& mesh : m_renderMeshes) {
        std::vector<Triangle> triangles = mesh->getTransformedTriangles();  // Получение трансформированных треугольников
        ...
        for (const auto& triangle : projectedTriangles) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                while (newTriangles > 0) {
                    ...
                    switch (i) {
                    case 0: poligonsToAdd = Triangle::clipAgainsPlane({0, 0, 0}, {0, 1, 0}, tri, clipped[0], clipped[1]); break;  // Верхняя граница
                    case 1: poligonsToAdd = Triangle::clipAgainsPlane({0, (float)glbl::window::height - 1, 0}, {0, -1, 0}, tri, clipped[0], clipped[1]); break;  // Нижняя граница
                    case 2: poligonsToAdd = Triangle::clipAgainsPlane({0, 0, 0}, {1, 0, 0}, tri, clipped[0], clipped[1]); break;  // Левая граница
                    case 3: poligonsToAdd = Triangle::clipAgainsPlane({(float)glbl::window::width - 1, 0, 0}, {-1, 0, 0}, tri, clipped[0], clipped[1]); break;  // Правая граница
                    }
