...
void Triangle::scalingToDisplay() {
    ...
    translateX(1.f);
    translateY(1.f);
    scaleX(0.5f * (float)config.width);
    scaleY(0.5f * (float)config.height);
}
...
sf::VertexArray Triangle::texturedTriangle(DepthBuffer& depthBuffer, sf::Image* texture) {
    ...
    if (texture && config.textureVisible) {
    ...
    if (dy1) {
        for (size_t i = y1; i <= y2; i++) {
            ...
            for (size_t j = ax; j < bx; j++) {
                ...
                if (texW > depthBuffer(i * config.width + j)) {
                    if (texture && config.textureVisible) {
                    ...
                    depthBuffer(i * config.width + j) = texW;
                }
