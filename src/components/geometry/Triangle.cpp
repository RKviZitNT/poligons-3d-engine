...
void Triangle::scalingToDisplay() {
    scaleX(-1);  // Инверсия по оси X
    scaleY(-1);  // Инверсия по оси Y
    translateX(1.f);  // Перемещение по оси X
    translateY(1.f);  // Перемещение по оси Y
    scaleX(0.5f * glbl::window::width);  // Масштабирование по оси X
    scaleY(0.5f * glbl::window::height);  // Масштабирование по оси Y
}
...
Vec3d Triangle::getNormal() const {
    Vec3d ab = p[1] - p[0];  // Вектор AB
    Vec3d ac = p[2] - p[0];  // Вектор AC
    return ab.cross(ac).normalize();  // Нормаль как векторное произведение AB и AC
}

// Проецирование треугольника (деление на w)
void Triangle::projectionDiv() {
    for (int i = 0; i < 3; i++) {
        p[i].projectionDiv();  // Деление координат вершины на w
        t[i].projectionDiv(p[i].w);  // Деление текстурных координат на w
        t[i].setW(1.f / p[i].w);  // Установка нового значения w
    }
}
...
Triangle Triangle::operator*(const Mat4x4& mat) {
    ...
    for (int i = 0; i < 3; i++) {
        result.p[i] = p[i] * mat;  // Применение матрицы к каждой вершине
    }
    return result;
}
...
Triangle& Triangle::operator*=(const Mat4x4& mat) {
    for (int i = 0; i < 3; i++) {
        p[i] = p[i] * mat;  // Применение матрицы к каждой вершине
    }
    return *this;
}
...
sf::VertexArray Triangle::texturedTriangle(DepthBuffer& depthBuffer, sf::Image* texture) {
    ...
    int   y1 = p[0].y, y2 = p[1].y, y3 = p[2].y;  // Y-координаты вершин
    int   x1 = p[0].x, x2 = p[1].x, x3 = p[2].x;  // X-координаты вершин
    float u1 = t[0].u, u2 = t[1].u, u3 = t[2].u;  // U-координаты текстур
    float v1 = t[0].v, v2 = t[1].v, v3 = t[2].v;  // V-координаты текстур
    float w1 = t[0].w, w2 = t[1].w, w3 = t[2].w;  // W-координаты (для перспективной коррекции)
    ...
    int   dy1 = y2 - y1;  // Разница по Y между первой и второй вершиной
    int   dx1 = x2 - x1;  // Разница по X между первой и второй вершиной
    float du1 = u2 - u1;  // Разница по U между первой и второй вершиной
    float dv1 = v2 - v1;  // Разница по V между первой и второй вершиной
    float dw1 = w2 - w1;  // Разница по W между первой и второй вершиной

    int   dy2 = y3 - y1;  // Разница по Y между первой и третьей вершиной
    int   dx2 = x3 - x1;  // Разница по X между первой и третьей вершиной
    float du2 = u3 - u1;  // Разница по U между первой и третьей вершиной
    float dv2 = v3 - v1;  // Разница по V между первой и третьей вершиной
    float dw2 = w3 - w1;  // Разница по W между первой и третьей вершиной

    float texU, texV, texW;  // Текущие текстурные координаты и W

    // Шаги для интерполяции
    float daxStep = 0, dbxStep = 0;  // Шаги по X для двух сторон треугольника
    float du1Step = 0, dv1Step = 0;  // Шаги по U и V для первой стороны
    float du2Step = 0, dv2Step = 0;  // Шаги по U и V для второй стороны
    float dw1Step = 0, dw2Step = 0;  // Шаги по W для двух сторон

    // Вычисление шагов интерполяции
    if (dy1) daxStep = dx1 / (float)std::abs(dy1);  // Шаг по X для первой стороны
    if (dy2) dbxStep = dx2 / (float)std::abs(dy2);  // Шаг по X для второй стороны

    if (dy1) du1Step = du1 / (float)std::abs(dy1);  // Шаг по U для первой стороны
    if (dy1) dv1Step = dv1 / (float)std::abs(dy1);  // Шаг по V для первой стороны
    if (dy1) dw1Step = dw1 / (float)std::abs(dy1);  // Шаг по W для первой стороны

    if (dy2) du2Step = du2 / (float)std::abs(dy2);  // Шаг по U для второй стороны
    if (dy2) dv2Step = dv2 / (float)std::abs(dy2);  // Шаг по V для второй стороны
    if (dy2) dw2Step = dw2 / (float)std::abs(dy2);  // Шаг по W для второй стороны
    ...
    sf::Color texCol;  // Цвет текстуры
    Color triCol;  // Цвет треугольника (если текстура не используется)
    ...
    // Отрисовка нижней части треугольника (от y2 до y3)
    dy1 = y3 - y2;
    dx1 = x3 - x2;
    du1 = u3 - u2;
    dv1 = v3 - v2;
...
int Triangle::clipAgainsPlane(const Vec3d& planePoint, const Vec3d& planeNormal, const Triangle& inTri, Triangle& outTri1, Triangle& outTri2) {
    ...
    // Массивы для хранения вершин и текстурных координат внутри и снаружи плоскости
    const Vec3d* insidePoints[3];  // Вершины внутри плоскости
    const Vec3d* outsidePoints[3];  // Вершины снаружи плоскости
    int insidePointCount = 0, outsidePointCount = 0;  // Количество вершин внутри и снаружи

    const Vec2d* insideTex[3];  // Текстурные координаты внутри плоскости
    const Vec2d* outsideTex[3];  // Текстурные координаты снаружи плоскости
    int insideTexCount = 0, outsideTexCount = 0;  // Количество текстурных координат внутри и снаружи
    ...
    if (insidePointCount == 1 && outsidePointCount == 2) {
        float t;  // Параметр для интерполяции
        ...
        return 1;  // Возвращаем один новый треугольник
    }
    ...
    if (insidePointCount == 2 && outsidePointCount == 1) {
        ...
        return 2;  // Возвращаем два новых треугольника
    }

    return true;  // Возвращаем true (хотя это значение не используется)
}