...
void Camera::update() {
    const float maxPith = glbl::hpi;  // Максимальный угол наклона по вертикали
    if (fPitch > maxPith) fPitch = maxPith;  // Ограничение угла наклона сверху
    if (fPitch < -maxPith) fPitch = -maxPith;  // Ограничение угла наклона снизу
